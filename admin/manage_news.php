...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $image = ($news && isset($news['image'])) ? $news['image'] : '';
    ...
    if (empty($title) || empty($content)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['size'] > 0) {
            ...
            if (!in_array($ext, $allowed)) {
            ...
            } elseif ($file['size'] > 20242880) { // 5MB limit
                ...
                if (move_uploaded_file($file['tmp_name'], $upload_path)) {
                    ...
                    if ($news && isset($news['image']) && $news['image']) {
                        $old_path = __DIR__ . '/../assets/uploads/news/' . $news['image'];
        ...
        if (!$error) {
            if ($id) {
                // Update existing
                $stmt = $pdo->prepare("UPDATE news SET title = ?, content = ?, image = ?, date_updated = CURRENT_TIMESTAMP WHERE id = ?");
